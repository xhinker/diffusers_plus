...
class IPAdapter:
    ...
    @torch.inference_mode()
    def get_prompt_embeds_lwp(self, images, negative_images=None, prompt=None, negative_prompt=None, weight=[]):
        prompt_embeds, negative_prompt_embeds = self.get_image_embeds(images, negative_images=negative_images)
        ...
        if prompt is not None:
            (
                text_prompt_embeds
                , text_prompt_neg_embeds
                , text_pooled_prompt_embeds
                , text_negative_pooled_prompt_embeds
            ) = get_weighted_text_embeddings_sdxl(
                self.pipe
                , prompt = prompt
                , neg_prompt = negative_prompt
            )
            prompt_embeds = torch.cat((text_prompt_embeds, prompt_embeds), dim=1)
            negative_prompt_embeds = torch.cat((text_prompt_neg_embeds, negative_prompt_embeds), dim=1)

        output = (prompt_embeds, negative_prompt_embeds)

        if self.is_sdxl:
            output += (text_pooled_prompt_embeds, text_negative_pooled_prompt_embeds)
